...
#     for latitute in range(20,55):
#         predicted_language = knn_classify(k, cities, [longitude, latitute])
#         plots[predicted_language][0].append(longitude)
#         plots[predicted_language][1].append(latitute)



# for language, (x,y) in plots.items():
#    plt.scatter(x,y,color=colors[language],marker=markers[language],label=language,zorder=10,s=2)
...
#                     if other_city!= city]
#         predicted_language = knn_classify(k, other_city, location)
...
def random_distances(dim,num_pairs):
    return [distance(random_points(dim), random_points(dim)) for _ in range(num_pairs)]
...
for dim in dimensions:
    ...
    print(dim, mean(distances), min(distances))
